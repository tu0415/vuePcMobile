	...
	<div class="list">
		...
		<div class="table" v-loading="loadingTable">
			<el-table :data="tableData" style="width: 100%">
				...
				<el-table-column align="center" label="标题" width="260">
					<template slot-scope="scope">
				...
				<el-table-column align="center" label="操作">
					<template slot-scope="scope">
						<a href="javascript:;" class="cancelCollect" @click="cancelCollectEvt(scope.row)">取消收藏</a>
					</template>
				</el-table-column>
			</el-table>
...
<script>
	...
	export default {
		...
		methods: {
			...
			cancelCollectEvt(data) {
				indexApi.delCollect({
					userId: this.userInfo.id,
					id: data.id
				}).then(d => {
					if (d.code == '0') {
						this.$showSuccessMessage('取消收藏成功!');
						this.getList(1);
					} else {
						this.$showErrorMessage(d.reason)
					}
